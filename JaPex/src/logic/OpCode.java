...
public class OpCode {
	public static void toMatch(String line) {
		...
		if (Main.currentStateNr == 0) {
			...
			Main.stateQueue.add(new State(line, -1, localVariables,
					new Stack<String>()));
			...
			String[] regexArray = new String[] { "iinc", "[adfil]load$",
					"[adfil]load_[0-3]", "[adfil]store", "[adfil]store_[0-3]",
					"[dfil]2[bcdilfs]$", "[ilfd]add$",
					"[fild]const_([0-5]|m1)", "[dfil]div$", "[dfil]mul$",
					"[dfil]neg", "[dfil]rem", "[dfil]sub", "pop|pop2", "new$",
					"sipush$", "ldc|ldc_w", "if(eq|ge|gt|le|lt|ne)$",
					"goto|goto_w", "[lfd]cmp([gl])*", "get(field|static)",
					"invoke(dynamic|special|static)", "instanceof",
					"if_[ai]cmp(eq|ne|ge|gt|le|lt)" };
			...
			case 0:
				iinc(line, Integer.valueOf(lineTokens[0].substring(0,
						...
						Integer.valueOf(lineTokens[2].substring(0,
								lineTokens[0].length() - 1)), lineTokens[3]);
				break;
	...
	private static void iinc(String line, int byteNr, int index, String constant) {
		State state = new State(line, byteNr, Main.stateQueue.get(
				Main.currentStateNr - 1).getLocalVariables(), Main.stateQueue
				.get(Main.currentStateNr - 1).getOperandStack());

		state.setLocalVariableElement(
				index,
				state.getLocalVariables().get(index) + " + "
